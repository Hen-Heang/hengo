...
import {
  recoveryCoachInputSchema,
  urgeEventInputSchema,
  weeklyReviewInputSchema,
  whenThenPlanInputSchema,
} from "./recovery-schemas"
import {
  DEFAULT_RECOVERY_NOTIFICATION_TEXT,
  recoveryNotificationText,
} from "./recovery-notifications"

const migration = readFileSync(
  join(process.cwd(), "supabase/migrations/20260719045208_recovery_workspace.sql"),
  "utf8",
)
...
describe("Recovery security contract", () => {
  ...
  it("requires explicit consent and strips fields outside the AI allowlist", () => {
    ...
    const parsed = recoveryCoachInputSchema.parse({
      consent: true,
      mode: "urge",
      intensity: 7,
  ...
  it("validates implementation intentions and AI review consent", () => {
    expect(
      whenThenPlanInputSchema.safeParse({ ifText: " ", thenText: "Take a walk" }).success,
    ).toBe(false)
    expect(
      weeklyReviewInputSchema.safeParse({
  ...
  it("fails unauthenticated AI coach requests before an AI call", async () => {
    ...
    const response = await recoveryCoachPost(
      new Request("http://localhost/api/ai/recovery-coach", {
        method: "POST",
        body: JSON.stringify({ consent: true, mode: "urge" }),
      }),
    )
