...
describe("recovery analytics", () => {
  ...
  it("supports frequency reduction without assuming missing days were safe", () => {
    const events = [
      event("s1", "h1", "2026-07-10T03:00:00Z", { kind: "slip" }),
      event("s2", "h1", "2026-07-17T03:00:00Z", { kind: "slip" }),
    ]
  ...
  it("never penalizes honest lapse reporting in the momentum formula", () => {
    ...
    const honest = recoveryMomentum(
      [event("s1", "h1", "2026-07-19T01:00:00Z", { kind: "slip", note: "One useful lesson" })],
      checkIns,
      now,
    )
  ...
  it("calculates time to return after a lapse", () => {
    const events = [
      event("s1", "h1", "2026-07-17T00:00:00Z", { kind: "slip" }),
      event("c1", "h1", "2026-07-17T06:00:00Z", { kind: "moment" }),
    ]
  ...
  it("detects the highest-risk two-hour window in the user's timezone", () => {
    const events = [
      event("e1", "h1", "2026-07-18T14:10:00Z"),
      event("e2", "h1", "2026-07-17T14:40:00Z"),
      event("e3", "h1", "2026-07-18T01:00:00Z"),
    ]
    expect(highestRiskTimeRange(events, "Asia/Seoul")).toEqual({
      startHour: 22,
      endHour: 0,
      count: 2,
    })
  ...
  it("ranks coping actions by recorded effectiveness", () => {
    const events = [
      event("e1", "h1", "2026-07-18T01:00:00Z", { actionTaken: "Walk", rodeOut: true }),
      event("e2", "h1", "2026-07-18T02:00:00Z", { actionTaken: "Walk", rodeOut: false }),
      event("e3", "h1", "2026-07-18T03:00:00Z", {
        actionTaken: "Korean review",
        healthyActionCompleted: true,
      }),
    ]
  ...
  it("keeps multiple targets isolated", () => {
    const events = [
      event("a", "h1", "2026-07-18T01:00:00Z", { kind: "slip" }),
      event("b", "h2", "2026-07-10T01:00:00Z", { kind: "slip" }),
    ]
  ...
  it("uses unique local dates for check-in completion", () => {
    const checkIns = [
      checkIn("a", "h1", "2026-07-19"),
      { ...checkIn("b", "h1", "2026-07-19"), period: "evening" as const },
      checkIn("c", "h1", "2026-07-18"),
    ]
