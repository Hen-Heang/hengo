...
export function registerCreateGoalTool(server: McpServer, ctx: McpContext): void {
  server.registerTool(
    "create_goal",
    {
      title: "Create goal",
      description:
        "Create one new goal owned by the signed-in user. Ask the user before calling this.",
    ...
    instrumentedTool(
      ...
      "write",
      "create_goal",
      async ({ title, description, targetDate, status }) => {
        ...
        if (existingId) {
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: `Already created "${goal.title}" moments ago — no duplicate created.`,
              },
            ],
