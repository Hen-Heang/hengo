...
export function registerCreateTaskTool(server: McpServer, ctx: McpContext): void {
  server.registerTool(
    ...
    instrumentedTool(
      ...
      "write",
      "create_task",
      async ({ title, description, goalId, startDate, endDate, durationMinutes, tags }) => {
        ...
        existingQuery = goalId
          ? existingQuery.eq("goal_id", goalId)
          : existingQuery.is("goal_id", null)
        ...
        if (existing) {
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: `Already created "${task.title}" moments ago — no duplicate created.`,
              },
            ],
        ...
        const { data, error } = await ctx.db
          ...
          .insert({
            ...
            ...taskStatusPatch(
              deriveStatusFromSchedule({ start_date: startDate, end_date: resolvedEndDate }, today),
            ),
          })
          .select(TASK_COLUMNS)
          .single()
        ...
        return {
          content: [
            { type: "text" as const, text: `Created task "${task.title}" (${task.status}).` },
          ],
