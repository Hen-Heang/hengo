...
export type RetrievedSourceType = "note" | "inbox_item" | "journal" | "memory" | "goal" | "correction" | "task" | "manual_activity"
 ...
 */
export async function retrieveMemoryContext(db: SupabaseClient, question: string): Promise<RetrievedItem[]> {
  const searchQuery = sanitizeSearchQuery(question)
  const ftsOptions = { type: "websearch" as const, config: "english" }

  const [notesRes, inboxRes, journalFtsRes, journalRecentRes, memoriesRes, goalsRes, correctionsRes, tasksRes, activitiesRes] =
    await Promise.all([
      searchQuery
        ? db
            .from("kori_notes")
      ...
      db
        ...
        .select("id, original_text, corrected_text, explanation, error_category, occurrence_count, last_seen_at")
  ...
  for (const row of (notesRes.data ?? []) as { id: string; slug: string; title: string; description: string; content: string }[]) {
    items.push({
      type: "note",
      id: row.id,
      title: row.title,
  ...
  for (const row of (inboxRes.data ?? []) as { id: string; title: string | null; content: string; item_type: string }[]) {
    items.push({
      type: "inbox_item",
      id: row.id,
  ...
  for (const row of (journalFtsRes.data ?? []) as Parameters<typeof pushJournalRow>[0][]) pushJournalRow(row)
  for (const row of (journalRecentRes.data ?? []) as Parameters<typeof pushJournalRow>[0][]) pushJournalRow(row)
  ...
  for (const row of (goalsRes.data ?? []) as {
    ...
    if (row.health_status) bits.push(`health: ${row.health_status}${row.health_reason ? ` (${row.health_reason})` : ""}`)
    if (row.outcome_statement) bits.push(row.outcome_statement)
    items.push({ type: "goal", id: row.id, title: row.title, snippet: snippet(bits.join(" — ")), href: `/goals/${row.id}` })
  }
  ...
  for (const row of (correctionsRes.data ?? []) as {
    ...
    items.push({
      ...
      snippet: snippet(`"${row.original_text}" → "${row.corrected_text}"${row.explanation ? ` (${row.explanation})` : ""}`),
  ...
  for (const row of (activitiesRes.data ?? []) as {
    ...
    items.push({
      ...
      id: row.id,
      title: row.label,
      snippet: snippet(`${row.occurred_at.slice(0, 10)}${row.category ? ` (${row.category})` : ""}${row.notes ? ` — ${row.notes}` : ""}`),
      href: "/timeline",
    })
  }
...
export function formatRetrievedContext(items: RetrievedItem[]): string {
  if (items.length === 0) return "<user_data>\n(No matching personal data was found for this question.)\n</user_data>"
  const lines = items.map(
    (item, i) => `[${i}] (${item.type}${item.id ? `, id=${item.id}` : ""}) ${item.title}: ${item.snippet}`,
  )
  return `<user_data>\n${lines.join("\n")}\n</user_data>`
}
