...
function readStoredLock(): StoredLock | null {
  ...
  try {
    return JSON.parse(localStorage.getItem(LOCK_STORAGE_KEY) ?? "null") as StoredLock | null
  } catch {
    return null
  }
}

export function hasRecoveryLockCredential(): boolean {
  return Boolean(readStoredLock())
}
export function isRecoveryUnlocked(): boolean {
  return typeof window !== "undefined" && sessionStorage.getItem(UNLOCK_SESSION_KEY) === "true"
}
export function lockRecovery(): void {
