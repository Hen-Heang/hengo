...
export function ListeningQuestionCard({
  ...
  const play = useCallback(
    async (rate: number, isReplay = true) => {
      ...
      <CardContent className="p-5 sm:p-6">
        ...
        <div className="mt-5 min-h-24 rounded-2xl border border-dashed border-border bg-muted/20 p-5">
          {revealedKorean ? (
            <p
              lang="ko"
              className="break-words text-[22px] font-semibold leading-[1.55] text-foreground sm:text-2xl"
            >
              {question.ko}
            </p>
          ) : (
            <div className="flex min-h-14 items-center justify-center text-center">
              <p className="max-w-md text-sm leading-relaxed text-muted-foreground">
                The Korean text is hidden. Listen for the topic and question ending first; replaying
                or revealing is a learning tool, not a failure.
              </p>
            </div>
          )}
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
          <Button
            ...
            aria-label="Play question normally"
          ...
          <Button
            ...
            onClick={() => play(0.75)}
            disabled={loading || playing}
            aria-label="Play question slowly"
            className="h-11 rounded-xl"
          >
          ...
          <Button
            type="button"
            variant="outline"
            onClick={revealKorean}
            disabled={revealedKorean}
            className="h-11 rounded-xl"
          >
            Reveal Korean
          </Button>
          <Button
            type="button"
            variant="ghost"
            onClick={() => setRevealedEnglish((value) => !value)}
            disabled={!question.en}
            className="h-11 rounded-xl"
          >
            <Languages className="mr-2 size-4" />
            {revealedEnglish ? "Hide English" : "Optional English"}
          </Button>
        </div>
