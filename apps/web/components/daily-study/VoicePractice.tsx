...
export function VoicePractice({
  ...
  const completedIds = useMemo(
    () => new Set(plan.attempts.filter((attempt) => attempt.successfulRetry).map((attempt) => attempt.questionId)),
    [plan.attempts],
  )
  const firstIncomplete = plan.content.spokenQuestions.findIndex((question) => !completedIds.has(question.id))
  ...
  return (
    <section aria-labelledby="voice-practice-title" className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 id="voice-practice-title" className="font-semibold">Voice role-play</h3>
      ...
      <div className="rounded-2xl border border-blue-500/20 bg-blue-500/5 p-4">
        <p className="text-xs font-medium text-muted-foreground">{plan.content.roleplay.scenario}</p>
        <div className="mt-3 flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="break-keep text-xl font-semibold leading-snug [overflow-wrap:anywhere]">{question.korean}</p>
            {plan.romanizationVisible && (
              <p className="mt-1 break-words text-xs text-blue-600 dark:text-blue-400">{question.romanization}</p>
            )}
      ...
      {showHint && <p className="rounded-xl bg-amber-500/10 p-3 text-sm text-amber-900 dark:text-amber-200">{question.hint}</p>}
      {showAnswer && (
        <div className="flex items-center justify-between gap-2 rounded-xl bg-muted p-3">
          <p className="break-keep text-sm font-medium [overflow-wrap:anywhere]">{question.modelAnswer}</p>
      ...
      {!speech.supported && (
        <p className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-sm text-amber-800 dark:text-amber-200">
          Speech recognition is unavailable in this browser. You can still type or edit the transcript below.
        </p>
      )}
      {(speech.error || error) && (
      ...
      {!feedback && (
        <div className="space-y-3">
          <label htmlFor="daily-voice-transcript" className="flex items-center gap-2 text-sm font-medium">
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex">
            <Button
              ...
              className="min-h-11"
            >
              {speech.status === "listening" ? <Square size={16} className="mr-2" /> : <Mic size={16} className="mr-2" />}
              {speech.status === "listening" ? "Stop" : "Record"}
            </Button>
            <Button type="button" disabled={evaluating || !speech.transcript.trim()} onClick={evaluate} className="min-h-11">
      ...
      {feedback && (
        <div className="space-y-3 rounded-2xl border border-border bg-card p-4">
          <div className="flex items-start gap-2">
            <CheckCircle2 className={feedback.communicationSuccessful ? "text-emerald-600" : "text-amber-600"} size={20} />
            <div>
          ...
          <p className="text-xs text-muted-foreground">
            Feedback covers grammar, meaning, vocabulary and naturalness from the transcript. It is not a pronunciation score.
          </p>
          {!questionComplete ? (
...
function FeedbackLine({ label, value, speak = false }: { label: string; value: string; speak?: boolean }) {
  return (
