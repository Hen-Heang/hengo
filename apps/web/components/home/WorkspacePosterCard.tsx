...
export function WorkspacePosterCard({
  ...
  return (
    ...
    >
      <div className={cn("pointer-events-none absolute -right-12 -top-12 h-48 w-48 rounded-full blur-[80px]", accent.glow)} />
        ...
        {stats.length > 0 && (
          <div className="mt-auto flex flex-wrap gap-2">
            {stats.map((stat) => (
              ...
              >
                <p className="font-mono text-base font-semibold tracking-tight text-foreground">{stat.value}</p>
                <p className="mt-0.5 text-xs font-medium leading-4 text-muted-foreground">{stat.label}</p>
              </div>
            ))}
          </div>
        )}

        <div className={cn("mt-auto flex min-h-11 items-center gap-1.5 text-sm font-semibold", accent.cta)}>
          {cta}
