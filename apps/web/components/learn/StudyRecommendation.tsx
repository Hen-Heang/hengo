...
import { nextReviewBatchSize, type DailyMission } from "@/lib/api"
...
export function StudyRecommendation({
  ...
  loading: boolean
  /** Vocabulary backlog size — what turns "done for today" into "here's more". */
  dueCount: number
  dueLoading: boolean
}) {
  ...
  if (!next) {
    // The daily plan being finished doesn't mean there's nothing to do — the
    // SRS backlog is usually still there. Only say "come back tomorrow" when
    // that's actually true.
    const nextBatch = nextReviewBatchSize(dueCount)
    return (
      <div className="rounded-3xl border border-emerald-500/20 bg-emerald-500/5 p-5 shadow-sm sm:p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {dueLoading
                ? "Checking what else is due…"
                : nextBatch > 0
                  ? `${dueCount.toLocaleString()} ${dueCount === 1 ? "word is" : "words are"} still due — extra reps are optional.`
                  : "You're all caught up. Come back tomorrow for a new plan."}
            </p>
          </div>
        </div>
        {!dueLoading && nextBatch > 0 && (
          <Link
            href="/vocab?review=session"
            className="mt-4 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 text-sm font-bold text-white shadow-sm shadow-emerald-600/20 transition-colors hover:bg-emerald-500 active:scale-[0.985] dark:bg-emerald-500 dark:hover:bg-emerald-400 sm:w-auto sm:px-6"
          >
