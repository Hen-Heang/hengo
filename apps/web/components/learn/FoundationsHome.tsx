          ...
          {lessons.map((lesson) => (
            <motion.div key={lesson.id} variants={item}>
              <Link
                ...
                className="group flex h-full flex-col gap-3 rounded-lg border border-border bg-card p-4 shadow-sm transition-all hover:border-blue-500/30 hover:shadow-md active:scale-[0.99] dark:bg-slate-900/40"
              >
