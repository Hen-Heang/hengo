...
export function PhraseCollectionCard({
  collection,
  cardCount,
  masteredCount,
  dueCount,
  onTogglePin,
}: PhraseCollectionCardProps) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-border bg-card p-5 shadow-sm dark:bg-slate-900/40">
      {onTogglePin && (
        ...
        >
          {collection.pinned ? (
            <Pin size={14} strokeWidth={2.5} className="fill-current" />
          ) : (
      ...
      <Link href={`/phrasebook/${collection.id}`} className="block">
        <h3 className="pr-8 text-lg font-bold leading-snug text-foreground hover:underline">
          {collection.titleEn}
        </h3>
