...
const STATUS_LABELS: Record<ProtectionStatus, string> = {
  not_set: "Not set",
  planned: "Planned",
  active: "Active",
...
export function ProtectionChecklist({
  ...
  onSave: (input: ProtectionItemInput) => Promise<unknown>
}) {
  ...
  const save = async (
    ...
    try {
      await onSave({
        ...
        status,
        preferredAction,
        sortOrder: SUGGESTIONS.indexOf(suggestion),
      })
    } finally {
  ...
  return (
    <section className="rounded-2xl border border-border bg-card p-5 shadow-sm">
      <p className="app-kicker">Environment protection</p>
      <h2 className="mt-1 text-lg font-semibold">Reduce easy access to triggers</h2>
      <p className="mt-1 text-sm leading-6 text-muted-foreground">
        These are configurable suggestions, not strict instructions. Mark what fits your life.
      </p>
      <div className="mt-6 space-y-6">
        {SECTIONS.map(({ category, label, icon: Icon }) => (
          <div key={category}>
            ...
            <div className="mt-3 divide-y divide-border rounded-2xl border border-border/70">
              {SUGGESTIONS.filter((item) => item.category === category).map((suggestion) => {
                const saved = itemFor(suggestion.label)
                const status = saved?.status ?? "not_set"
                return (
                  <div
                    key={suggestion.label}
                    className="grid gap-3 p-3.5 sm:grid-cols-[1fr_10rem_auto] sm:items-center"
                  >
                    <p className="text-sm leading-5">{suggestion.label}</p>
                    <Select
                      disabled={savingLabel === suggestion.label}
                      value={status}
                      onValueChange={(value) =>
                        save(suggestion, value as ProtectionStatus, saved?.preferredAction ?? false)
                      }
                    >
                    ...
                    <label className="flex items-center justify-between gap-2 text-xs text-muted-foreground sm:justify-start">
                      <Switch
                        checked={saved?.preferredAction ?? false}
                        onCheckedChange={(checked) => save(suggestion, status, checked)}
                        aria-label={`Use ${suggestion.label} in Urge Rescue`}
                      />
                      Rescue
                    </label>
                  </div>
