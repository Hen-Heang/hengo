...
const JWT = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMTExMTExMS0xMTExLTQxMTEtODExMS0xMTExMTExMTExMTEiLCJleHAiOjIwMDAwMDAwMDB9.signature"
...
const questions = Array.from({ length: 50 }, (_, index) => ({
  ...
  sample_answer_ko: "\uD55C\uAD6D\uC758 \uC5EC\uB984\uC740 \uB35C\uACE0 \uC2B5\uD569\uB2C8\uB2E4. \uCE84\uBCF4\uB514\uC544\uBCF4\uB2E4 \uBE44\uAC00 \uB9CE\uC2B5\uB2C8\uB2E4.",
...
async function installMocks(page: Page) {
  ...
  await page.addInitScript(({ userId, token }) => {
    window.localStorage.setItem("koriai-auth", JSON.stringify({
      access_token: token,
    ...
    if (table === "kori_interview_question_progress") {
      if (request.method() === "GET") {
        ...
        if (questionFilter?.startsWith("eq.")) {
          ...
          await route.fulfill({ status: row ? 200 : 406, contentType: "application/json", body: JSON.stringify(row ?? null) })
        } else {
          await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify([...progress.values()]) })
        }
        return
      }
  ...
  await page.route("**/api/ai/interview/speaking-check", async (route) => {
    ...
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({
      scores: { speaking: score, grammar: score, vocabulary: score, pronunciation: score, confidence: retry ? 4 : 3, naturalness: score },
      feedback: "Use two short, complete sentences.",
      correctedAnswer: "\uD55C\uAD6D\uC758 \uC5EC\uB984\uC740 \uB35C\uACE0 \uC2B5\uD569\uB2C8\uB2E4.",
      betterAlternative: "\uD55C\uAD6D\uC758 \uC5EC\uB984\uC740 \uB35C\uACE0 \uC2B5\uD574\uC11C \uC870\uAE08 \uD798\uB4ED\uB2C8\uB2E4.",
...
test("dashboard, listening, correction, retry, question bank, and mobile layout", async ({ page }) => {
  await installMocks(page)
  await page.goto("http://127.0.0.1:3000/interview/speaking")
  await expect(page.getByRole("heading", { name: "Practice the five questions that matter today" })).toBeVisible()
  ...
  await expect(page.getByText("Microphone permission is blocked. Allow microphone access in the browser and try again.")).toBeVisible()
  ...
  await transcript.fill("\uD55C\uAD6D \uC5EC\uB984\uC740 \uB35C\uACE0 \uC2B5\uD569\uB2C8\uB2E4. \uADF8\uB7EC\uB098 \uC9C0\uAE08\uC740 \uC801\uC751\uD588\uC2B5\uB2C8\uB2E4.")
  await page.getByRole("button", { name: "Submit confirmed transcript" }).click()
  ...
  for (let questionNumber = 2; questionNumber <= 5; questionNumber += 1) {
    await expect(transcript).toBeEditable()
    await transcript.fill(`\uD55C\uAD6D \uC5EC\uB984 ${questionNumber}\uBC88 \uB2F5\uBCC0\uC785\uB2C8\uB2E4.`)
    await page.getByRole("button", { name: "Submit confirmed transcript" }).click()
    await expect(page.getByText("Correction available")).toBeVisible()
    if (questionNumber < 5) {
      await page.getByRole("button", { name: "Next Question" }).click()
